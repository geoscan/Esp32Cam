...
static int avi_add_chunk(avi_t *AVI, char *tag, char *data, int length)
{
	char c[8];
	...
	if (fwrite( c, 1,  8, AVI->fdes) != 8 ||
		fwrite( data, 1,  length, AVI->fdes) != length)
	{
...
avi_t *AVI_open_output_file(char *filename)
{
	...
	for (i = 0; i < HEADERBYTES; i++)
		AVI_header[i] = 0;
	i = fwrite( AVI_header, 1,  HEADERBYTES, AVI->fdes);
	if (i != HEADERBYTES)
	{
		fclose(AVI->fdes);
		AVI_errno = AVI_ERR_WRITE;
...
#define AVIF_COPYRIGHTED 0x00020000
	...
	if (fseek(AVI->fdes, 0, SEEK_SET) < 0 ||
		fwrite( AVI_header, 1,  HEADERBYTES, AVI->fdes) != HEADERBYTES /*||
...
avi_t *AVI_open_input_file(char *filename, int getIndex)
{
	...
	if (fread( data, 1,  12, AVI->fdes) != 12)
	...
	while (1)
	{
		...
		if (strncasecmp(data, "LIST", 4) == 0)
		{
			...
			if (strncasecmp(data, "hdrl", 4) == 0)
			{
				...
				if (fread( hdrl_data, 1,  n, AVI->fdes) != n)
					ERR_EXIT(AVI_ERR_READ)
			}
		...
		else if (strncasecmp(data, "idx1", 4) == 0)
		{
			...
			AVI->idx = (char((*)[16]))malloc(n);
			if (AVI->idx == 0)
				ERR_EXIT(AVI_ERR_NO_MEM)
			if (fread( AVI->idx, 1,  n, AVI->fdes) != n)
				ERR_EXIT(AVI_ERR_READ)
		}
		else
			fseek(AVI->fdes, n, SEEK_CUR);
		...
		while (1)
		{
			...
			if (((data[2] == 'd' || data[2] == 'D') &&
				...
				((data[2] == 'w' || data[2] == 'W') &&
				 (data[3] == 'b' || data[3] == 'B')))
			{
				avi_add_index_entry(AVI, data, 0, fseek(AVI->fdes, 0, SEEK_CUR) - 8, n);
			}
...
long AVI_read_audio(avi_t *AVI, char *audbuf, long bytes)
{
	...
	while (bytes > 0)
	{
		...
		if (left == 0)
		{
			if (AVI->audio_posc >= AVI->audio_chunks - 1)
				return nr;
			AVI->audio_posc++;
			AVI->audio_posb = 0;
			continue;
		...
		fseek(AVI->fdes, pos, SEEK_SET);
		if (fread( audbuf + nr, 1,  todo, AVI->fdes) != todo)
		{
			AVI_errno = AVI_ERR_READ;
			return -1;
		}
	...
	while (1)
	{
		...
		else if (strncasecmp(data, AVI->audio_tag, 4) == 0)
		{
			*len = n;
			...
			if (fread( audbuf, 1,  n, AVI->fdes) != n)
				return 0;
			return 2;
			break;
		}
